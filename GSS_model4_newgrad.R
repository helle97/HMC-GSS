...
LogLik_Rposdef_drift_posterior <- function(Y, tree, theta, D = Des_mat, C = C_mat, C_inv = C_mat_inv){
  n <- dim(Y)[1]
  p <- dim(Y)[2]
  ...
  deg_free <- 5
  Sigma0 <- (matrix(c(var(Y[1:n])/2, cov(Y[1:n], Y[(n+1):length(Y)]), cov(Y[1:n],Y[(n+1):length(Y)]), var(Y[(n+1):length(Y)])/2), nrow =2))
  S <- (1/deg_free)*Sigma0 #*(1/deg_free)
...
g_LogLik_Rposdef_drift_posterior_simple <- function(Y, tree, theta){
  #print(theta)
  list_side <- rep(NA, 9)
  ...
  return(numDeriv::grad(func = LogLik_Rposdef_drift_posterior, x = theta, method = "simple", method.args = list(side= list_side), tree = tree, Y = Y))
}
g_LogLik_Rposdef_drift_posterior_simple(Y = X, tree =  tree, theta = params)
...
system.time(model4_hmc_simpleg <- hmclearn::hmc(N, theta.init = theta,
                                  epsilon = eps_vals, L = L,
                                  logPOSTERIOR = LogLik_Rposdef_drift_posterior,
                                  glogPOSTERIOR = g_LogLik_Rposdef_drift_posterior_simple,
                                  varnames = c("mu1", "mu2", "gamma11", "gamma22", "R11", "R12", "R22", "v1", "v2"),
                                  param = list(Y = Y, tree = tree), chains = 2))

summary(model4_hmc_simpleg, burnin = 150)
plot(model4_hmc_simpleg, burnin = 150)
model4_accept_hmc_new <- model4_hmc_simpleg$accept/N
model4_hmc_rhat <- summary(model4_hmc_simpleg, burnin = 150)[,8]
diagplots(model4_hmc_simpleg, burnin=150, comparison.theta=c(4,6,NA,NA,NA,NA, NA, -0.2, 0.2))
diagplots(model4_hmc_simpleg, burnin = 150, plotfun = 1)
...
GSS_HMC <- function(Y, tree, N, K, beta, model_log_posterior, model_gradient, initial_params, L, eps_vals ){
  ...
  log_q_beta <- function(Y, tree, theta, beta, sm = sample_means, sd = sample_sd,
                         model_log_posterior = LogLik_Rposdef_drift_posterior, D =Des_mat, C = C_mat, C_inv = C_mat_inv){
...
eps_vals <- c(0.02, 0.02, rep(0.002, 2), rep(0.00003,3), 0.02, 0.02)
L = 15
N = 500
K = 10
...
system.time(log_r4 <-GSS_HMC(Y = X, tree = tree, N = N, K, beta = beta, 
        ...
        model_gradient = g_LogLik_Rposdef_drift_posterior_simple,
        initial_params = params, L = L, eps_vals = eps_vals ))

log_r4

save.image(file='GSS_model4_new.RData')
