...
LogLik_simple_drift_posterior <- function(Y, tree, theta, D = Des_mat, C = C_mat){
  n <- dim(Y)[1]
  p <- dim(Y)[2]
...
g_LogLik_simple_drift_posterior <- function(Y, tree, theta){
  list_side <- rep(NA, 8)
  ...
  return(numDeriv::grad(func = LogLik_simple_drift_posterior, x = theta, method = "Richardson", method.args = list(side= list_side), tree = tree, Y = Y))
}
g_LogLik_simple_drift_posterior(Y = X, tree =  tree, theta = params)
...
set.seed(143)
L <- 15
eps_vals <- c(0.01, 0.01, 0.002, 0.002, 0.002, 0.002, 0.01, 0.01)
length(eps_vals)
N <- 500
theta <- params


system.time(model2_hmc <-hmclearn::hmc(N, theta.init = theta,
                          epsilon = eps_vals, L = L,
                          logPOSTERIOR = LogLik_simple_drift_posterior,
                          glogPOSTERIOR = g_LogLik_simple_drift_posterior,
                          varnames = c("mu1", "mu2", "gamma11", "gamma22", "R11", "R22", "v1", "v2"),
                          param = list(Y = Y, tree = tree), chains = 2))


summary(model2_hmc, burnin = 150)
plot(model2_hmc, burnin = 150)
model2_accept_hmc <- mean(model2_hmc$accept/N)
model2_hmc_rhat <- summary(model2_hmc, burnin = 150)[,8]
model2_hmc_rhat
diagplots(model2_hmc, burnin = 150, comparison.theta = c(4,6,NA,NA,NA,NA,-0.2, 0.2))
...
GSS_HMC <- function(Y, tree, N, K, beta, model_log_posterior, model_gradient, initial_params, L, eps_vals){
  ...
  g_LogLik_simple_drift_posterior <- model_gradient
  ...
  MCMC_init <- function(theta){
    foreach(times=1:3,
            .combine = rbind,
            .export = c('model_log_posterior', 'model_gradient', 'LogLik_simple_drift_posterior','g_LogLik_simple_drift_posterior',
  ...
  for (k in (2:K)){ # We sum for beta_1 in the estimator which is the same as beta[2] 
    ...
    if (k == K){
      ...
      MCMC_samples <- function(theta){
        foreach(times=1:3,
                        .combine = rbind,
                        .export = c('log_q_beta', 'g_log_q_beta', 'LogLik_simple_drift_posterior','g_LogLik_simple_drift_posterior',
...
unregister_dopar()
system.time(log_r_m2 <- GSS_HMC(Y = X, tree = tree, N = N, K, beta = beta, 
        model_log_posterior = LogLik_simple_drift_posterior,
        model_gradient = g_LogLik_simple_drift_posterior,
        initial_params = params, L = L, eps_vals = eps_vals ))

save.image(file='GSS_model2_FINAL.RData')
