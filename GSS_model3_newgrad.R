...
LogLik_Rposdef_posterior <- function(Y, tree, theta, D = Design_matrix, C= C_mat){
  n <- dim(Y)[1]
  p <- dim(Y)[2]
...
g_LogLik_Rposdef_posterior_simple <- function(Y, tree, theta){
  list_side <- rep(NA, 7)
  ...
  if (theta[7]<=0.09){ list_side[6] <- +1} 
  # check limits of covariances, param must be between 0 and 1
  if (theta[6]<=0.09){ list_side[4] <- +1}
  if (theta[6]>=0.91){ list_side[4] <- -1} 
  return(numDeriv::grad(func = LogLik_Rposdef_posterior, x = theta, method = "simple", method.args = list(side= list_side), tree = tree, Y = Y))
}
g_LogLik_Rposdef_posterior_simple(Y = X, tree =  tree, theta = params)


L <- 20
#eps_vals <- c(0.01, 0.01, rep(0.001, 2), rep(0.00005,3))
...
system.time(model3_sim_results_simpleg <- hmclearn::hmc(N, theta.init = theta,
                                 ...
                                 glogPOSTERIOR = g_LogLik_Rposdef_posterior_simple,
                                 varnames = c("mu1", "mu2", "gamma11", "gamma22", "R11","R12", "R22"),
                                 param = list(Y = Y, tree = tree), chains = 3))


summary(model3_sim_results_simpleg, burnin = 150)
plot(model3_sim_results_simpleg, burnin = 150)
model3_accept_hmc_new <- model3_sim_results_simpleg$accept/N
...
GSS_HMC <- function(Y, tree, N, K, beta, model_log_posterior, model_gradient, initial_params, L, eps_vals){
  ...
  MCMC_init <- function(theta){
    foreach(times=1:3,
            .combine = rbind,
            .export = c('model_log_posterior', 'model_gradient', 'LogLik_Rposdef_posterior','g_LogLik_Rposdef_posterior_simple',
  ...
  ref_dist <- function(theta, sm = sample_means, sd = sample_sd, obs = Y){
    ...
    R_log_lik <- LaplacesDemon::dwishart(Omega = matrix(c(theta[5], theta[6], theta[6], theta[7]), nrow =2), nu= p + 1, S = S, log = TRUE)
  ...
  log_q_beta <- function(Y, tree, theta, beta, sm = sample_means, sd = sample_sd,
                         model_log_posterior = LogLik_Rposdef_posterior, ref = ref_dist, D =Design_matrix, C = C_mat){
    return(beta * model_log_posterior(Y, tree, theta, D, C) + (1-beta)*log(ref(theta)))
  }
  ...
  calc_frac <- function(Y, tree, theta, log_posterior = model_log_posterior, ref = ref_dist, D = Design_matrix, C = C_mat){
  ...
  for (k in (2:K)){ # We sum for beta_1 in the estimator which is the same as beta[2] 
    ...
    if (k == K){
      ...
      MCMC_samples <- function(theta){
        foreach(times=1:3,
                .combine = rbind,
                .export = c('log_q_beta', 'g_log_q_beta', 'LogLik_Rposdef_posterior','g_LogLik_Rposdef_posterior_simple',
  ...
  return(list(log_r = log_rhat, eta_vector = eta_vec, r_vector = r_vec))
}


params <- c(rnorm(1,5,0.1), rnorm(1,5, 0.1), 0.2, 0.2,
            0.2, 0.1, 0.2)
eps_vals <- c(0.02, 0.02, rep(0.002, 2), rep(3e-5, 3))
L = 15
N = 500
K = 10
...
system.time(log_r <- GSS_HMC(Y = X, tree = tree, N = N, K, beta = beta, 
        ...
        model_gradient = g_LogLik_Rposdef_posterior_simple,
        initial_params = params, L = L, eps_vals = eps_vals))

log_r


save.image(file='GSS_model3_new.RData')
